...
// ❌ WRONG: .first() gets stuck on iteration 1
const page = $('Loop Node').first().json;  // Always page 1!

// ✅ CORRECT: .item gets current iteration
const page = $('Loop Node').item.json;  // Page 1, then 2, then 3...

// ✅ CORRECT: Reference previous loop step
...
```javascript
// ❌ WRONG: Manual context management (unnecessary and error-prone)
let accumulated = this.getContext('results') || [];
accumulated.push(currentResult);
this.setContext('results', accumulated);

// ✅ CORRECT: n8n automatically tracks all executions
...
```javascript
// ❌ RISKY: In debug mode, may show old cached data
...
```
┌─────────────────────────────────────────────┐
│ Inside Loop (processing current iteration) │
├─────────────────────────────────────────────┤
│ • Loop node data:     .item                 │
│ • Previous loop step: .item                 │
│ • Pre-loop config:    .first()              │
│ • Input to this node: $input                │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│ After Loop (Done branch - collecting)      │
├─────────────────────────────────────────────┤
│ • All loop results:   .all()                │
│ • Original structure: .first()              │
│ • Merge/aggregate:    .all().map()          │
└─────────────────────────────────────────────┘
```
