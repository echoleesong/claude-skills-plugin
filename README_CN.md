...
本插件目前包含 **14 个专业技能**，涵盖 n8n 工作流开发和 **Skill 生命周期管理**：

### Skill 生命周期管理（3 个技能）- **全新**

11. **skill-factory** - 自动化 Skill 工厂
    - 将任意 GitHub 仓库一键转换为标准化 Skill
    - 自动获取仓库元数据（commit hash、README）
    - 生成符合规范的目录结构和扩展元数据

12. **skill-manager** - Skill 生命周期管理器
    - 并发检查所有 Skills 的更新状态
    - 支持批量扫描、更新、删除操作
    - 生成状态报告（outdated/current/unmanaged）

13. **skill-evolution** - Skill 经验进化管理器
    - 将对话中的经验教训持久化到 `evolution.json`
    - 智能缝合：自动将经验写入 SKILL.md
    - 跨版本保留：Skill 更新后经验不丢失
...
10. **skill-lookup** - Skill 发现和安装工具
    - 搜索和发现可用的 Skills
    - 安装和管理 Skills

14. **更多技能即将推出** - 本插件设计为可扩展的
...
**Skill 生命周期管理：**
- **从 GitHub 创建 Skill**：`/skill-factory <url>` 或 "将这个仓库打包成 Skill"
- **检查更新**：`/skill-manager check` 或 "检查我的 Skills 是否有更新"
- **列出 Skills**：`/skill-manager list` 或 "列出我的 Skills"
- **记录经验**：`/skill-evolution` 或 "复盘一下刚才的对话"
...
**Skill 生命周期管理：**
```
"将 https://github.com/yt-dlp/yt-dlp 打包成 Skill"
→ 激活：skill-factory
...
#### skill-factory
自动化 Skill 工厂，将 GitHub 仓库转换为标准化 Skill。当用户提供 GitHub URL 并希望"打包"、"封装"或"创建 Skill"时使用。自动获取仓库元数据、生成标准目录结构、注入生命周期管理所需的扩展元数据。

#### skill-manager
Skill 生命周期管理器。用于批量扫描 Skills 目录、检查 GitHub 更新、执行版本升级、管理 Skills 库存。支持 `/skill-manager check`、`/skill-manager list`、`/skill-manager delete` 等命令。

#### skill-evolution
Skill 经验进化管理器。在对话结束时分析用户反馈，提取最佳实践和解决方案，持久化到 `evolution.json` 并缝合到 SKILL.md。当用户说"复盘"、"记录经验"、"保存这个技巧"时使用。
...
├── skills/
│   ├── skill-factory/       # Skill 工厂（新）
│   ├── skill-manager/       # Skill 管理器（新）
│   ├── skill-evolution/     # Skill 进化器（新）
│   ├── n8n-workflow-patterns/
│   ├── n8n-code-javascript/
│   ├── n8n-code-python/
│   ├── n8n-expression-syntax/
...
│   └── md-to-pptx/
├── scripts/                 # 独立可执行脚本（新）
│   ├── check_updates.py     # 批量检查更新
│   ├── batch_evolve.py      # 批量对齐经验
│   └── validate_all.py      # 批量验证元数据
├── docs/                    # 文档
...
本插件提供独立可执行的 Python 脚本，支持 CI/CD 和批量操作：
